...
def generateJSON(userInput=str, acID='TML', name='formulario'):    
    ...
    otus_gemini = os.getenv('GEMINI_API_KEY')
    form = {
    ...
    client = genai.Client(api_key=otus_gemini)
    response = client.models.generate_content(
        model="gemini-2.0-flash",
        config=types.GenerateContentConfig(
            system_instruction="""You are a helpful AI assistant capable of generating JSONs based on user input. These JSONs are responsible for creating forms in an external appication. Your task is to generate and add JSONs objects to a list inside ***itemContainer*** field in the following JSON: ***{"itemContainer":[]}***. The following are JSON itens options to be added to the list as specified by user input. Fill the correct properties inside the JSON structure with user input:
        ...
        contents=f"Generate a JSON with {userInput}. Do it step by step as specified by the system structure. Return it as a raw JSON, without spaces."
    )

    itemCont = json.loads(response.text) #loads generated itemContainer field json to variable
